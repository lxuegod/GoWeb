...
type Engine struct {
	*RouterGroup
	...
	htmlTemplates *template.Template //	为html提供  	将所有的模板加载进内存
	funcMap       template.FuncMap   //	为html提供	所有的自定义模板渲染函数
}
...
//	createStaticHandler 创建静态handler
...
//	Static 服务静态文件  暴露给用户使用，用户可以将磁盘上的某个文件夹root映射到路由relativePath
