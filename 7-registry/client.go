...
func (client *Client) Call(ctx context.Context, serviceMethod string, args, reply interface{}) error {
	...
	select {
	//	TODO 提供一个人供用户自定义的 具备超时监测能力德context对象控制、
	case <-ctx.Done():
...
func NewHTTPClient(conn net.Conn, opt *Option) (*Client, error) {
	_, _ = io.WriteString(conn, fmt.Sprintf("CONNRCT %s HTTP/1.0\n\n", defaultRPCPath))
...
//	DialHTTP 连接到指定网络地址的服务器，监听默认HTTP RPC路径
